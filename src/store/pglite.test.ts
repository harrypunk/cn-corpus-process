...
describe("PgliteSink", () => {
  it("inserts raw records; insert-only staging means re-runs duplicate", async () => {
    ...
    const batch: RawRecord[] = [
      { author: "李璟", title: "应天长", content: "一钩初月临妆镜", corpusId: 1 },
      { author: "太宗皇帝", title: "帝京篇十首 一", content: "秦川雄帝宅", corpusId: 2 },
    ];
    ...
    const result = await check.query(
      'SELECT corpus_id AS "corpusId", author, title, content FROM raw_records ORDER BY id',
    );
    await check.close();
