...
原始记录写入 `raw_records(id, corpus_id, author, title, content)`——`corpus_id` 为语料整数标记
（常量见 `src/corpus.ts`，与上游目录名无关），insert-only staging，重跑会重复，dedup 归归一化阶段。
`ETL_SINK_KIND` 选择 sink（`src/store/`）：
